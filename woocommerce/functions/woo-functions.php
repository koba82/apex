...
/**
 * Class extending WC_Widget_Layered_Nav_Filters
 *
 * Adds option to select the order in which the attributes appear in on the frontend.
 *
 */

class WC_Widget_Layered_Nav_Filters_Apex extends WC_Widget_Layered_Nav_Filters {


    public function countActiveAttributes() {
            ...
            if ( ! empty( $_chosen_attributes ) ) {
                foreach ( $_chosen_attributes as $taxonomy => $data ) {
                    foreach ( $data['terms'] as $term_slug ) {
                        ...
                        echo '<li class="someclass ' . esc_attr( implode( ' ', $filter_classes ) ) . '"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . esc_html( $term->name ) . '</a></li>';
                    }
                }
            }
...
add_action( 'widgets_init', 'replace_layered_nav_filters', 11 );

function replace_layered_nav_filters() {
    unregister_widget( 'WC_Widget_Layered_Nav_Filters' );
    register_widget( 'WC_Widget_Layered_Nav_Filters_Apex' );
}
